...
u1_D = Function(V)  # For left boundary (x = 0)
#u2_D = Function(V)  # For right boundary (x = 1)

# Assign values to these functions
u1_D.interpolate(lambda x: np.zeros((domain.geometry.dim, x.shape[1])))
#u2_D.interpolate(lambda x: np.vstack((-0.01 * np.ones(x.shape[1]), np.zeros((domain.geometry.dim - 1, x.shape[1])))))

# Create boundary conditions
bc1 = dirichletbc(u1_D, fem.locate_dofs_topological(V, fdim, clamped_boundary1_facets))
#bc2 = dirichletbc(u2_D, fem.locate_dofs_topological(V, fdim, clamped_boundary2_facets))


# Create XDMF file for output
xdmf = io.XDMFFile(domain.comm, "elasticity_2d_e5.xdmf", "w")
...
if dim ==2:
        f = fem.Constant(domain, PETSc.ScalarType((-0.1, 0.0)))
elif dim ==3:
...
for i in range(num_steps):
    ...
    #############################displacement_x = -0.01 * (i + 1)
    f = fem.Constant(domain, PETSc.ScalarType((-0.1*(i+1), 0.0)))
    a = ufl.inner(sigma(u), epsilon(v)) * ufl.dx
    L = ufl.dot(f, v) * ufl.dx
    ##u2_D = np.array([displacement_x, 0.0, 0.0], dtype=PETSc.ScalarType)
    ##########################u2_D.interpolate(lambda x: np.vstack((displacement_x * np.ones(x.shape[1]), np.zeros((domain.geometry.dim - 1, x.shape[1])))))
